...
* **To serve the existing build without recompiling:**
  ...
  *(This simply starts the Firebase local server from the existing `public/` folder).*
...
1. **Global Config:** The `config.json` in the root directory holds the `defaultLang` and `supportedLangs` configuration.
